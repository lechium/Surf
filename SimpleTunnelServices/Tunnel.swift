...
open class Tunnel: NSObject {
    ...
    func writeDataToTunnel(_ data: Data, startingAtOffset: Int) -> Int {
        myLog(message: "writeDataToTunnel called on abstract base class")
        return -1
    }
	...
	func serializeMessage(_ messageProperties: [String: AnyObject]) -> Data? {
		...
		do {
			...
			myLog(message: "Failed to create a data object from a message property list: \(messageProperties)")
		}
	...
	func sendMessage(_ messageProperties: [String: AnyObject]) -> Bool {
        ...
        guard let messageData = serializeMessage(messageProperties) else {
            myLog(message: "Failed to create message data")
            return false
        }
        ...
        if written < messageData.count {
            savedData.append(data: messageData as NSData, offset: written)
	...
	func sendData(_ data: Data, forConnection connectionIdentifier: Int) {
		let properties = createMessagePropertiesForConnection(connectionIdentifier: connectionIdentifier, commandType: .data, extraProperties:[
				TunnelMessageKey.Data.rawValue : data as AnyObject
			])

		if !sendMessage(properties) {
			myLog(message: "Failed to send a data message for connection \(connectionIdentifier)")
		}
	}
	...
	func sendSuspendForConnection(_ connectionIdentifier: Int) {
		let properties = createMessagePropertiesForConnection(connectionIdentifier: connectionIdentifier, commandType: .suspend)
		if !sendMessage(properties) {
			myLog(message: "Failed to send a suspend message for connection \(connectionIdentifier)")
		}
	}

	/// Send a Resume message on the tunnel connection.
	func sendResumeForConnection(_ connectionIdentifier: Int) {
		let properties = createMessagePropertiesForConnection(connectionIdentifier: connectionIdentifier, commandType: .resume)
		if !sendMessage(properties) {
			myLog(message: "Failed to send a resume message for connection \(connectionIdentifier)")
		}
	}
	...
	open func sendCloseType(_ type: TunnelConnectionCloseDirection, forConnection connectionIdentifier: Int) {
		let properties = createMessagePropertiesForConnection(connectionIdentifier: connectionIdentifier, commandType: .close, extraProperties:[
				TunnelMessageKey.CloseDirection.rawValue: type.rawValue as AnyObject
			])
			
		if !sendMessage(properties) {
			myLog(message: "Failed to send a close message for connection \(connectionIdentifier)")
		}
	}
	...
	func sendPackets(_ packets: [Data], protocols: [NSNumber], forConnection connectionIdentifier: Int) {
		let properties = createMessagePropertiesForConnection(connectionIdentifier: connectionIdentifier, commandType: .packets, extraProperties:[
				TunnelMessageKey.Packets.rawValue: packets as AnyObject,
				TunnelMessageKey.Protocols.rawValue: protocols as AnyObject
			])

		if !sendMessage(properties) {
			myLog(message: "Failed to send a packet message")
		}
	}
	...
	func handlePacket(_ packetData: Data) -> Bool {
		...
		do {
			...
			myLog(message: "Failed to create the message properties from the packet")
			return false
		}

		guard let command = properties[TunnelMessageKey.Command.rawValue] as? Int else {
			myLog(message: "Message command type is missing")
			return false
		}
		guard let commandType = TunnelCommand(rawValue: command) else {
			myLog(message: "Message command type \(command) is invalid")
			return false
		}
		...
		switch commandType {
			case .data:
				...
				{
					myLog(message: "Received data for connection \(connection?.identifier) from \(host):\(port)")
			...
			case .close:
				...
				{
					myLog(message: "\(connection?.identifier): closing \(closeDirection)")
					targetConnection.closeConnection(closeDirection)
				} else {
					myLog(message: "\(connection?.identifier): closing reads and writes")
	...
	func handleMessage(_ command: TunnelCommand, properties: [String: AnyObject], connection: Connection?) -> Bool {
		myLog(message: "handleMessage called on abstract base class")
		return false
	}
}
