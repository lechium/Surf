	...
	func updateConnectionRulesCell() {
		...
		if let rulesIndexPath = self.getIndexPathOfCell(connectionRulesCell) {
			...
			if targetRule.action != .evaluateConnection {
				cells.remove(at: rulesIndexPath.row)
			...
			if targetRule.action == .evaluateConnection {
				cells.insert(connectionRulesCell, at: actionIndexPath.row + 1)
				let indexPaths = [ IndexPath(row: actionIndexPath.row + 1, section: actionIndexPath.section) ]
